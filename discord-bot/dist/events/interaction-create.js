import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, Colors, EmbedBuilder, MessageFlags, ModalBuilder, PermissionFlagsBits, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
import * as fs from 'node:fs';
import * as path from 'node:path';
import { fileURLToPath } from 'node:url';
import { config } from '../config.js';
import { getFruits, getOrders, createOrder, updateOrderStatus } from '../utils/api.js';
import { fruitStockEmbed, ticketEmbed, ticketWelcomeEmbed } from '../utils/embeds.js';
...
async function cmdStock(interaction) {
...
async function cmdPedido(interaction) {
    ...
    const fruit = fruitsResult.data.find((f) => f.name.toLowerCase() === fruitName.toLowerCase());
    if (!fruit) {
...
async function cmdEntregar(interaction) {
    if (!isStaff(interaction.member)) {
        return safeReply(interaction, 'Solo el staff puede usar este comando.');
    }
...
async function cmdPedidos(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .map((o) => `\`${o.id.slice(0, 8)}\` — **${o.customerName}** — S/ ${o.total.toFixed(2)} — ${o.paymentMethod ?? 'N/A'}`)
        .join('\n'))
        .setTimestamp();
    await interaction.editReply({ embeds: [embed] });
}
...
async function handleOpenTicket(interaction) {
    ...
    if (!guild)
        return;
    const existing = guild.channels.cache.find((ch) => ch.type === ChannelType.GuildText &&
...
async function handleSelectMenu(interaction) {
    if (interaction.customId === 'ticket_fruit_select') {
        ...
        const emailInput = new TextInputBuilder()
            ...
            .setStyle(TextInputStyle.Short)
            .setRequired(true);
        modal.addComponents(new ActionRowBuilder().addComponents(quantityInput), new ActionRowBuilder().addComponents(nameInput), new ActionRowBuilder().addComponents(discordInput), new ActionRowBuilder().addComponents(robloxInput), new ActionRowBuilder().addComponents(emailInput));
