    ...
    <main className="relative min-h-screen pt-24 pb-20">
      ...
      <div className="pointer-events-none fixed inset-0 -z-20">
        <div
          ...
          style={{
            backgroundImage: "url('/backgrounds/pueblo.png')",
      ...
      <section className="relative overflow-hidden pb-6 sm:pb-10">
        <div className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 lg:px-8">
          ...
          <div className="pointer-events-none absolute left-1/2 top-0 -z-10 -translate-x-1/2">
            <div className="size-[600px] rounded-full blur-3xl sm:size-[700px]"
              style={{ background: 'radial-gradient(circle, rgba(239,35,60,0.08) 0%, transparent 70%)' }}
            />
          </div>
          ...
          >
            {/* Badge */}
            <motion.span
              className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-red-400 shadow-[0_0_20px_-8px_rgba(239,35,60,0.3)] backdrop-blur-sm"
            ...
            {/* Title */}
            <motion.h1
              className="mt-4 flex flex-wrap items-center justify-center gap-x-4 font-black uppercase leading-[1.15] md:flex-nowrap"
              style={{
                fontFamily: 'var(--font-heading), sans-serif',
                textShadow: '0 4px 48px rgba(0,0,0,0.6), 0 0 60px rgba(239,35,60,0.08)',
            ...
            >
              <p className="mx-auto mt-4 text-[15px] leading-relaxed text-zinc-300 sm:whitespace-nowrap sm:text-base">
                ...
                <div className="relative group">
                  <div className="relative flex items-center rounded-xl border border-white/[0.07] bg-black/60 backdrop-blur-sm transition-all duration-300 focus-within:border-red-500/40 focus-within:shadow-[0_0_30px_-10px_rgba(239,35,60,0.3)] sm:rounded-2xl">
                    ...
                    <Input
                      ...
                      className="h-13 border-0 bg-transparent pl-12 pr-4 text-[15px] text-white placeholder:text-zinc-500 focus-visible:ring-0 focus-visible:outline-none sm:h-14 sm:pl-13 sm:text-base"
                      aria-label="Buscar frutas"
                    />
                    {query && (
                      <button
                        onClick={() => setQuery('')}
                        className="mr-2 flex size-6 items-center justify-center rounded-full bg-white/10 text-xs text-zinc-400 transition-colors hover:bg-white/20 hover:text-white"
                      >
                        ✕
                      </button>
                    )}
                ...
                <div className="mt-5 flex items-center justify-center gap-3 pb-1">
                  {filters.map((f, i) => (
                    <motion.button
                      ...
                      type="button"
                      onClick={() => setActive(f)}
                      className="relative rounded-full px-6 py-2.5 text-sm font-semibold uppercase tracking-wider transition-all"
                      whileTap={{ scale: 0.95 }}
                      initial={{ opacity: 0, y: 12 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      {active === f ? (
                        <motion.div
                          layoutId="filter-bg"
                          className="absolute inset-0 rounded-full border border-red-500/40 bg-red-500/15 shadow-[0_0_20px_-8px_rgba(239,35,60,0.3)]"
                      ...
                      <span className={`relative z-10 flex items-center gap-1.5 ${
                        active === f ? 'text-red-400' : 'text-zinc-400'
                      }`}>
                        {f === 'Mythical' && <Sparkles className="size-3.5" />}
                        {f}
                      </span>
                    </motion.button>
                  ))}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {results.length > 0 ? (
          ...
          <motion.div
            className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-white/10 bg-white/5 py-16 text-center"
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
          >
            <PackageSearch className="size-8 text-zinc-500" />
            <p className="font-medium text-white">
              No hay frutas que coincidan con tu busqueda
            </p>
